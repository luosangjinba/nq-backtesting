...
### ADR-V7-001 / R13.1 — Branch-Local Draft At Formation Time

When this position was formed, a feature branch proposed an Annotation
foundation with stable Artifact ids, market anchors, relations, no-future
provenance, and Chart drill-down. Its specification is intentionally not copied
into `main`; this memo survives independently and does not accept ADR-V7-001.
If a later accepted foundation changes that terminology or owner model, the
Research candidate must adapt rather than create a private semantic store.
