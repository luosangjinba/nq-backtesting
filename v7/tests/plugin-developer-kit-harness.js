...
assert.equal(harnessRules.currentStep, 'P1a');
...
assert.equal(discovered.schemas.length, 13);
assert.equal(discovered.catalogs.length, 9);
assert.deepEqual(discovered.catalogs.find(({ path: logicalPath }) => logicalPath === 'catalogs/operations.json')
  .value.operations.map(({ id }) => id), [
