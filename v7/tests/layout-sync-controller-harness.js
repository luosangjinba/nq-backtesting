...
function makePorts({ failPersistence = false } = {}) {
  ...
  return {
    ...
    adapter: {
      ...
      setTimeSync(value) { calls.push(['time', value]); },
    },
    persist(value) {
...
assert.deepEqual(acceptedPorts.calls, [['adapter', false], ['time', false], ['view', false]]);
assert.equal(accepted.change('crosshair', true), true);
assert.equal(accepted.read().crosshair, true);
assert.deepEqual(acceptedPorts.calls.slice(-4), [
  ['persist', true], ['adapter', true], ['time', false], ['view', true],
], 'persistence must complete before an accepted projection');

assert.equal(accepted.change('time', true), true);
assert.equal(accepted.read().time, true);
assert.deepEqual(acceptedPorts.calls.slice(-4), [
  ['persist', true], ['adapter', true], ['time', true], ['view', true],
], 'accepted Time policy must reach only the adapter projection boundary');
...
assert.deepEqual(failedPorts.calls.slice(-5), [
  ['persist', true],
  ['adapter', false],
  ['time', false],
  ['view', false],
