...
| --- | --- | --- | --- | --- | --- | --- |
| NQ/ES SMT | Use Main=NQ and Comparison=ES on the same timeframe; create or review SMT evidence; select and locate it from Inspector. | SMT renders on primary and comparison, selection opens Inspector, locate lands on expected time. | Need old Split to see SMT, selection misses comparison object, or locate only works on secondary. | Technical pass only. | `smt-selection-smoke` and `comparison-window-browser-smoke` passed selection/Inspector/locate coverage; a real SMT review pass is still required before Split removal. | needs real-use data |
| 1M + HTF replay | Use primary 1M with comparison 1H or 4H while Replay On. | Comparison HTF candle progresses without showing future complete candle; cursor/hover sync is usable. | Future HTF data appears early, chart desyncs, or comparison becomes visually misleading. | Technical pass only. | `comparison-replay-sync-smoke` and browser smoke passed progressive replay path/source requests; user-visible replay review still needed before Split removal. | needs real-use data |
...
| Replay History restore | Save a replay state with Comparison Window enabled, reload, and restore from History. | Primary cursor/range and comparison instrument/timeframe/window state restore. | Comparison state is missing, stale, or loads wrong range. | Technical pass only. | `replay-history-comparison-smoke` passed comparison state identity/restore coverage; a real reload/restore review remains required before Split removal. | needs real-use data |
