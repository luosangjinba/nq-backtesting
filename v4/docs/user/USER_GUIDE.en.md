...
Comparison Window is the supported sliding comparison view. It is not a resize patch on the old Split Screen. The comparison chart sits on the left and the main chart remains visible on the right. Dragging the comparison right boundary changes the visible comparison width, and the chart relayouts inside that width so Lightweight Charts can keep its native right price scale at the Main/Comparison boundary. Legacy saved floating workspaces can still be normalized for compatibility, but new/reset windows use sliding mode. The old Split user entry points have been removed after the focused real-use audit passed.
