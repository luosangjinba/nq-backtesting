"""Stable, cheap identity for the currently active DuckDB market dataset.

The revision is transport/cache provenance, not a schema version.  It changes
when the database file is replaced or modified, including an active DuckDB WAL,
without hashing the (potentially multi-gigabyte) database on every request.
"""
...
REVISION_ALGORITHM = "v4-duckdb-stat-v1"
...
class DatasetRevisionUnstable(RuntimeError):
    """The database changed repeatedly while a bounded read was in progress."""
...
def resolve_dataset_revision(db_path: str, table_name: str) -> str:
    """Return one revision that remains stable until the DuckDB dataset changes."""
...
def resolve_dataset_revision_if_ready(db_path: str, table_name: str) -> Optional[str]:
    """Return ``None`` while a bootstrap deployment has no market database yet."""
