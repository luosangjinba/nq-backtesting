...
- [ ] Step 330: Large module decomposition backlog。目标是在 Comparison Window 行为稳定后，按风险顺序拆分长期大文件，避免在当前 UI 稳定期做纯重构扩大回归面。每次拆分必须保持用户行为不变，并配套现有 smoke 或新增窄覆盖。
  ...
  - [ ] Step 330.4: Comparison controller extraction plan：在 `replay-controls.js` 稳定后，拆 `v4/src/ui/comparison-window-controller.js`，候选边界为 DOM/render/header controls、sliding geometry/resize/drag、comparison data loading/replay source sync、main/comparison crosshair sync；保持 `initComparisonWindowController()` 作为唯一 app 入口。
