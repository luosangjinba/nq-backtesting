...
- TODO Split Screen 剩余扩展执行顺序：1) 做完整验证：node check、headless Chrome 覆盖默认 1H、副图 layout 切换不 reload、crosshair/replay cursor 并存、segment/composite/PDA overlay、Split 关闭清理、主图标注/segment/Inspector/replay 无回归；2) 后续如需同步 selected/linked highlight，再使用 resolver 的 `highlightPdaIds`
...
- TODO Viewport reset 命名收敛：当前工作完成后，删除旧 `Reset chart view` 语义和独立 `Scroll to latest` UI，把现有 `Scroll to latest` 行为改名为 `Reset chart view`；实现时保留 `scrollToLatest()` 行为（保持当前缩放、锚定最新 K 线、保留右侧 offset、重置价格轴 autoScale），UI 只保留一个 `Reset chart view` 按钮，`Alt+R` 也调用该行为，以贴近 TradingView
...
- 2026-05-26: Reaction Evidence 新方案确认：不做系统自动判断 `respect/sweep`，改为人工确认事件存在并指定 PDA + 连续 K 线群，系统只做客观量化；FVG respect 以 FVG 本体高度为分母计算 wick/body 进入百分比，body 超过 100% 不截断、如实保存并在 UI 标红；liquidity sweep 以参与 K 线群整体涨跌幅为分母计算 wick/body sweep 百分比；第一版用 Inspector 时间输入指定 first/last/terminal bar，不做 canvas 框选、不做自动识别、不做 verdict
