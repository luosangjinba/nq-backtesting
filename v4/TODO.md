...
- [ ] Step 288: Live Record lifecycle / review workflow。目标是在 Step 285-287 的 Order Setup 同构 Live Record 基础上，补齐真正的 journal lifecycle：显式状态流转、关闭/取消/复盘、Detail review 输入、Calendar lifecycle row/menu、可选 Order Setup 关联复盘、持久化/Review JSON 审计和浏览器验收；继续禁止 standalone `Live Orders`。计划见 `v4/sessions/session_20260614_live_record_lifecycle_review_plan.md`。
  ...
  - [ ] Step 288.10: Review JSON / persistence audit：审计并必要时扩展 Review JSON import/export，确保 lifecycle/review/result/reasons/linked setup/instrument scope 可保存恢复；若范围过大则明确 defer。
  - [ ] Step 288.11: Focused and browser verification：覆盖 create -> execution writes -> close/cancel/review -> review notes -> link/unlink setup -> Calendar status/menu -> persistence reload -> no standalone panel，并跑 live/order/browser smoke 与 `git diff --check`。
  - [ ] Step 288.12: Documentation and closeout：更新 TODO/session/必要用户文档，记录 lifecycle 语义、工作流、Order Setup 关系、非目标、验证结果；最终 clean worktree。
