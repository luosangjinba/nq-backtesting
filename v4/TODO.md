...
- TODO Split Screen 剩余扩展执行顺序：1) 做完整验证：node check、headless Chrome 覆盖默认 1H、副图 layout 切换、crosshair/replay cursor 并存、segment/composite/PDA overlay、Split 关闭清理、主图标注/segment/Inspector/replay 无回归；2) 若 overlay 继续扩展 selected/linked highlight，再抽 shared primitive builder，避免主副图 renderer 分叉扩大
