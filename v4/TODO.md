...
- TODO Split Screen: 增加副图 readonly overlay sync，同步显示现有 PDA / segment / composite，但不允许副图编辑、hit-test、右键菜单、selection 或 Inspector。建议新增独立副图 renderer（先 `secondary-segment-renderer.js` 同步 segment/composite，再 `secondary-pda-renderer.js` 同步 PDA），只读取现有 store 并渲染到副图 chart/series；不要直接复用当前硬绑定主图的 renderer，应抽公共 render-time / primitive builder 或让副图 renderer 显式传入 chart context；segment endpoint 可调用 `getSegmentPointRenderTime(point, secondaryTf)`，PDA timestamp 用 `getBucketStart(timestamp, secondaryTf)`；显隐逻辑先跟随主图 Display Mode resolver，第一版只 normal 样式，后续再同步 selected/linked highlight
- TODO Split Screen 剩余扩展执行顺序：1) 新增 `secondary-segment-renderer.js` 同步显示 segment/composite，只读、无 hit-test/selection/Inspector，第一版 normal 样式；2) 新增 `secondary-pda-renderer.js` 同步显示 PDA，先 liquidity-line/range，再 point-set/fib，仍保持只读；3) 做完整验证：node check、headless Chrome 覆盖默认 1H、副图 layout 切换、crosshair/replay cursor 并存、segment/composite/PDA overlay、Split 关闭清理、主图标注/segment/Inspector/replay 无回归
