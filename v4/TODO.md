...
- [ ] Step 330: Large module decomposition backlog。目标是在 Comparison Window 行为稳定后，按风险顺序拆分长期大文件，避免在当前 UI 稳定期做纯重构扩大回归面。每次拆分必须保持用户行为不变，并配套现有 smoke 或新增窄覆盖。
  ...
  - [ ] Step 330.2: Replay controls extraction plan：为 `v4/src/ui/replay-controls.js` 制定拆分边界，优先抽离纯 helper/restore snapshot/history bridge/DOM rendering/action handlers，保持对外 API 和事件名不变；拆前补齐 replay history restore、progressive replay、comparison sync、viewport reset 的 smoke 基线。
