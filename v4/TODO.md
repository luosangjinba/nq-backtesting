...
- [x] Step 282: Journal MVP data model / UI scope。目标是在当前 V4 复盘平台上平行出 journal 系统，优先定义临场记录的数据模型和最小 UI 范围；不先做行情源自动化，不复用 Order Setup 作为 journal order log。初始研究重点：临场状态、当时想法、计划/冲动、实际订单、执行纪律、情绪/身体状态、复盘后对照。初版设计文档已起草：`v4/docs/design/JOURNAL_MVP_DESIGN.md`，明确 Journal Day、Live Trade Log、Ideal Trade Review、Discipline Review 四层模型，并补充多层计划框架与 Phase A 设计冻结细化步骤。最新决策：JournalDay 按 account/day，Trade 按 instrument；LiveTradeLog 支持 fills/partial；PnL/size/R 第一版手填；Journal 作为 `index.html` app shell 内的独立 workspace。Step 282.A 已补 6 个 validation examples：真实违规亏损、按计划模拟、无交易但理想 missed trade、真实+模拟混合、赚钱但纪律差、多合约 partial/final exit；样例均可映射当前模型。Phase A 已冻结，可进入 Phase B minimal Journal workspace/store 实现。

- [x] Step 285: Journal Phase B - Minimal Daily Journal。目标是在 Phase A 冻结后实现最小可用 Journal Day，不做 LiveTradeLog/fills/IdealTradeReview/statistics。计划见 `v4/sessions/session_20260612_journal_phase_b.md`。
...
- [x] Step 290: Journal Execution Adapter。目标是在不改变 UI、不迁移 localStorage 的前提下，把当前 `JournalDay.liveTrades[]` 投影为 `JournalExecution` read model，为后续 linked setup summary 和 Actual Trades UI rework 做准备。范围：新增 `toJournalExecution` / `getJournalExecutions` 等 adapter；兼容 `orderReviewId` 与 legacy `linkedOrderSetupIds[0]`；保留 unlinked fallback snapshot；补 smoke/unit 覆盖 linked、legacy linked、unlinked 三类记录。已完成：新增 `journal-execution-adapter`，`normalizeLiveTradeLog` 保留可选 `orderReviewId`，新增 adapter smoke 覆盖 unlinked/direct linked/legacy linked/fill copy 行为。计划见 `v4/sessions/session_20260612_journal_execution_adapter.md`。

- [x] Step 291: Linked Setup Summary Read Model。目标是把 `JournalExecution.orderReviewId` 和 Backtesting Setup Set 只读连接起来，为后续 Journal Actual Trades UI 提供 linked/unlinked display model。已完成：新增 `journal-execution-setup-summary`，输出 `linked` / `missing-linked-setup` / `unlinked` 三种状态；linked execution 从 Backtesting setup-set 读取 instrument/direction/entry/stop/targets/result/summary，missing/unlinked 使用 `unlinkedSnapshot` fallback；新增 smoke 覆盖 direct linked、legacy linked、missing link、unlinked。计划见 `v4/sessions/session_20260612_journal_execution_setup_summary.md`。

- [x] Step 292: Journal Actual Trades UI Rework。目标是让 Journal Actual Trades 使用 `JournalExecutionDisplayModel`，linked trade 显示 Backtesting setup summary，Journal 只编辑 execution/fills/PnL/R/discipline/reflection；保持 unlinked execution 可用。计划见 `v4/sessions/session_20260612_journal_actual_trades_ui_rework.md`。
...
- [x] Step 296: Journal Linking Phase Review / Merge Readiness。目标是在 Step 290-295 完成后做合并前收口。已完成：先 push 当前分支，确认分支与 origin 同步；复跑 journal adapter、linked setup summary、setup candidates、order setup、journal browser smoke、全量 `v4/src`/`v4/tests` JS syntax、`git diff --check` 和敏感信息扫描；全部通过。当前阶段可进入 merge review，broker import、auto-match、create setup、migration、自动 PnL/R 和 statistics 继续暂缓。计划见 `v4/sessions/session_20260612_journal_actual_trades_ui_rework.md`。
