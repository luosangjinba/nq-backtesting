...
- [ ] Step 330: Large module decomposition backlog。目标是在 Comparison Window 行为稳定后，按风险顺序拆分长期大文件，避免在当前 UI 稳定期做纯重构扩大回归面。每次拆分必须保持用户行为不变，并配套现有 smoke 或新增窄覆盖。
  ...
  - [ ] Step 330.3: Replay controls staged extraction：按一次一个职责迁移，避免同时移动状态机和 UI。每个子提交只做 move/extract，不改变行为；每步跑 replay history、secondary/progressive replay、comparison window browser smoke 和 `git diff --check`。
