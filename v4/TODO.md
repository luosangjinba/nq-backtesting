...
- [ ] Step 330: Large module decomposition backlog。目标是在 Comparison Window 行为稳定后，按风险顺序拆分长期大文件，避免在当前 UI 稳定期做纯重构扩大回归面。每次拆分必须保持用户行为不变，并配套现有 smoke 或新增窄覆盖。
  - [ ] Step 330.1: Module responsibility audit：记录 500+ 行模块的职责、公共导出、事件订阅、DOM 依赖和现有测试覆盖；先确认哪些文件只是长、哪些文件是真正多职责。初始候选包括 `replay-controls.js`、`comparison-window-controller.js`、`inspector-sidebar.js`、`review-archive.js`、`tradovate-performance-importer.js`、`manual-annotation.js`。
