...
- 2026-06-08 Step 269 plan: Chart Notes mousemove hit-test 降负载。对 `chart-note-selection` 的 `mousemove` 做 RAF throttle，并把 chart note hit-test geometry/layout 缓存到 notes/bars/timeframe/replay/expanded 变化时才重建，避免每个鼠标事件都重建 bar map、过滤 notes、计算坐标与文本布局。验收：连续鼠标移动时最多每帧一次 hit-test；Chart Note hover/click 行为不变；`node --check`、browser smoke 通过。
- 2026-06-08 Step 270 plan: Replay overlay 重绘降负载。审查 `replay:changed` 触发的 PDA / Segment / Chart Notes primitive 全量 teardown/rebuild，优先做 visible/replay date 过滤与 RAF 合并，同一帧内多次事件只重绘一次；暂不做大规模 primitive diff。验收：Replay 播放时 overlay 视觉不丢失，PDA/Notes/Segments 在 replay cursor 前后显示口径不变，快速 replay 不出现持续多次 redundant render。
