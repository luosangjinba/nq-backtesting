...
- [ ] Step 330: Large module decomposition backlog。目标是在 Comparison Window 行为稳定后，按风险顺序拆分长期大文件，避免在当前 UI 稳定期做纯重构扩大回归面。每次拆分必须保持用户行为不变，并配套现有 smoke 或新增窄覆盖。
  ...
  - [ ] Step 330.6: Inspector and domain module triage：评估 `inspector-sidebar.js`、`review-archive.js`、`tradovate-performance-importer.js`、`manual-annotation.js` 等 900+ 行模块是否值得拆；只在有明确职责边界和测试入口时推进，避免为了行数做机械拆分。
