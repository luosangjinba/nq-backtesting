...
- [ ] Step 353: FX Replay-style session runtime。目标是彻底放弃“Date Range = 显示全部/加载全部范围”的主图逻辑，改成 FX Replay：Date Range 只定义 replay session 边界；进入图表后最新/最右 K 线就是 `sessionStart`，左侧只加载当前屏幕所需前缀；除非 replay next/play 推进 cursor，否则不请求、不缓存、不渲染 `cursor` 右侧未来 K 线；左拖时按需向前加载 prefix chunk；到 `sessionEnd` 停止 replay。记录见 `v4/sessions/session_20260626_step353_fx_replay_session_model.md`。
  ...
  - [x] Step 353.2: 新增 session state 与 chunk policy，但暂不接 UI。已新增 `ui/replay/replay-session-state.js`，建立独立 `ReplaySessionState`/store helpers，记录 instrument、timeframe、sessionStart、sessionEnd、cursor、autoUpdateEnd、mode、loadedChunks、visibleChunkRange；实现 initial prefix / previous prefix / next-forward-bar 规划，并拒绝非 immediate replay reveal 原因加载 cursor 右侧 bars；新增 `replay-session-state-smoke.js` 并纳入 `smoke_all.py --suite local`。
  - [x] Step 353.3: Step 351 bypass gate。已为 active replay session 加显式旁路保护：新增 active replay session registry，Toolbar/Calendar/Viewport controls/Replay History/Comparison/Time Reaction 在 session active 时不得继续用 `outerRange`、`resolveWindowAroundTimestamp()`、`resolveAdjacentWindow()` 或 14 天 virtual window 驱动主图显示；Prev/Next Window 在 session 中隐藏或禁用；共享 chunk loader 可保留，但不得写回 `requestedOuterRange` 作为 session 显示模型。
  - [x] Step 353.4: 替换 Date Range 主图入口。Toolbar 与 Calendar Date Range 主入口已改为创建 replay session，不再加载完整 range，也不再加载旧 14 天 window；首次只请求以 `sessionStart`/cursor 结尾的屏幕可见 prefix window；API padding 与 cursor 右侧 bars 会被过滤后再写入 primary store；active session 下主图右侧定位到 prefix 末端并显示 replay cursor。
  - [x] Step 353.5: 主图 all-in replay bars。active session 下 primary store 只保存 session-visible prefix bars，主图 series 只消费这些 replay-visible bars；`getReplayVisibleBars()` / `getReplayCursorTimestamp()` 已支持 active session，Calendar/PDA/notes 等现有 replay-aware 读取会使用 session 可见 bars；打开 session 后 emit `replay:changed`，Replay controls 显示只读 session 状态并禁用旧 first/back/play/forward/jump 动作；退出 session 通过 Replay close 显式清空 session/bars，不静默显示完整 Date Range。
  - [x] Step 353.6: 左拖 prefix 懒加载。已新增 active replay session prefix loader：监听主图 visible logical range，用户向左拖动接近已加载数据左边缘时，按当前 timeframe 请求更早 bounded prefix chunk，prepend/merge 并跳过重复/in-flight chunk；不因 long Date Range 自动加载 prefix；prepend 后平移 logical range 保持用户左拖视图稳定；远离视窗 chunk 释放策略留待后续性能调优。
  - [x] Step 353.7: Replay forward append。Replay Bar 的 next/play 已成为 active session 下唯一加载未来 K 线的路径；按需请求 cursor 右侧下一根 replay bar，不能超过 `sessionEnd`；请求结果只有成为新 cursor bar 时才进入 active replay buffer，API padding 中其它 future bars 不保留；遇到交易间隙时只 reveal 下一根真实 bar；到 sessionEnd 停止并提示 session finished。
  - [x] Step 353.8: 周期切换接入。active replay session 下切换 1M/5M/15M/1H/1D 时保留 sessionStart/sessionEnd/cursor，清空旧 timeframe chunks，重新请求以 cursor 结尾的可见 prefix window；继续禁止显示/请求 cursor 右侧未来 bars；非 session 路径保留旧 timeframe reload 行为。
  - [x] Step 353.9: Pane/Comparison/Calendar/overlays 收口。active session 下 Comparison/Pane 1 不再清空或走 Date Range resolve，而是按主图当前 session-visible 小窗口加载并过滤到 cursor；Calendar locate 不再为未加载目标开新窗口，未来日期直接拒绝，历史日期必须已经在 prefix 中；Replay History restore 改为重新打开 replay session，checkpoint 保存 sessionStart/sessionEnd；time overlays/price overlays 继续只读取当前 display bars，不触发完整 range 加载。
  - [x] Step 353.10: Browser smoke。已新增 `v4/tests/replay-session-browser-smoke.js`，在真实 headless Chrome 中 stub `/v4/bars` 并走前端 replay session 模块，覆盖一年 1M session 初始请求 bounded 且 end=start/cursor、初始右边界在 cursor、无请求以 sessionEnd 作为初始加载 end、加载更早 prefix、replay forward 每次只请求/显示下一根 bar、active replay buffer 不保留 cursor 右侧 future padding、到 end 停止、切换 1D 仍不显示 cursor 右侧 bars。
