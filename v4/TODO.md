...
- [x] Step 290: Manual Roll Calendar Workflow。目标是把 ES/NQ 换季 roll date 维护从一次性推断改成可重复的手动确认流程：系统用 raw contract volume 扫描候选切换日期并提醒，用户按实际交易切换确认，roll calendar 记录 `manual_validated` / `volume_validated` 后才允许写入；先解决 `NQH6 -> NQM6`，同一流程也适用于后续 ES/NQ 季度切换。已完成 scanner、reminder report、manual confirmation workflow、write guard allowlist、NQ preflight、用户文档、focused tests 和最终验证；`NQH6 -> NQM6` 已解决为 `2026-03-16 / volume_validated`，但 full default NQ refresh 仍因其他 non-write-eligible roll segments intentionally blocked，未执行 NQ 写库。计划见 `v4/sessions/session_20260614_manual_roll_calendar_workflow_plan.md`。
