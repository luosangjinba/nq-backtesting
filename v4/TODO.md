...
- [ ] Step 330: Large module decomposition backlog。目标是在 Comparison Window 行为稳定后，按风险顺序拆分长期大文件，避免在当前 UI 稳定期做纯重构扩大回归面。每次拆分必须保持用户行为不变，并配套现有 smoke 或新增窄覆盖。
  ...
  - [ ] Step 330.5: Comparison controller staged extraction：先抽无状态 DOM/render helpers，再抽 geometry/drag，最后抽 data loading 与 crosshair sync；每步跑 `comparison-window-browser-smoke`，并覆盖 restored-open workspace、Inspector resize、native price axis、viewport controls、Replay History comparison persistence。
