...
- [ ] Step 292: Architecture Review Fix Pack。目标是吸收 `v4/docs/user/v4-architecture-review.md` 中高价值发现，优先做小而可验证的正确性/安全修复：修 `/v4/price` instrument 静默错价、验证 maintenance-window 聚合口径、给 Data Maintenance POST 增加请求级 guard、明确 SMT persistence 策略；暂缓大文件拆分、persistence factory 和全局命名重构。计划见 `v4/sessions/session_20260616_architecture_review_fix_plan.md`。
  ...
  - [ ] Step 292.2: Fix `/v4/price` instrument scope：后端 `_handle_price` 读取并透传 `instrument`；前端 `fetchPrice()` 传 current Main instrument；加 focused smoke，确认 ES/NQ 同 timestamp 查价不串。
  - [ ] Step 292.3: Verify maintenance-window filtering consistency：只读比较 `/v4/daily_bars` 聚合和 `generate_daily_regime_csv.py` 的 17:00-18:00 维护时段排除口径，覆盖普通日和 DST 边界；仅在发现真实 mismatch 时改代码。
  - [ ] Step 292.4: Add Data Maintenance request guard：为 `/v4/data_maintenance/run` 增加最小自定义 header/CSRF-style guard，保留现有 action allowlist、确认串、mutex 和 localhost 边界。
  - [ ] Step 292.5: Decide SMT persistence status：审计 SMT 创建、history、Review JSON、Calendar visibility；决定补 dedicated localStorage persistence 或明确记录为 review-archive/history-scoped。
  - [ ] Step 292.6: Regression and closeout：跑 price/Data Maintenance/SMT/maintenance-window 相关 focused tests 与 `git diff --check`，更新 TODO/session，明确 P3 架构重构继续 deferred。
