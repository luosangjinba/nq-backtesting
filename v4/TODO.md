...
- [ ] Step 332: Server-centered multi-device sync short-term plan。目标是先用一台服务器集中承载 V4 API、数据库/数据目录、刷新任务和导入输出，让多台电脑访问同一个 server URL，解决主要数据源不同步问题；短期不做完整 workspace/localStorage 同步。计划见 `v4/docs/planning/server_sync_short_term_plan.md`。后续想法已写入该文档 Future Backlog。
