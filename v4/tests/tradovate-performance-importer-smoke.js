...
assert.equal(enhancedEs.execution.orders[0].quantity, 1, 'entry order includes original order quantity');
assert.equal(enhancedEs.execution.orders[0].filledQuantity, 1, 'filled entry order includes filled quantity');
assert.equal(enhancedEs.execution.orders[1].quantity, 1, 'stop order includes original order quantity');
assert.equal(enhancedEs.execution.orders[1].filledQuantity, 1, 'filled stop order includes filled quantity');
assert.equal(enhancedEs.execution.orders[2].quantity, 1, 'cancelled target order includes original order quantity');
assert.equal(enhancedEs.execution.orders[2].filledQuantity, null, 'cancelled target order keeps empty filled quantity as null');
...
assert.equal(normalizedEnhancedEs.execution.orders[1].quantity, 1, 'normalized enhanced orders preserve quantity');
assert.equal(normalizedEnhancedEs.execution.orders[1].filledQuantity, 1, 'normalized enhanced orders preserve filled quantity');
