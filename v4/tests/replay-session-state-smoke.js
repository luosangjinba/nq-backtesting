...
assert.equal(forward.request.start, '2026-06-01 21:55');
assert.equal(forward.request.end, '2026-06-01 21:56');

const moved = updateReplaySession(session, { cursor: forward.request.endTs });
assert.equal(moved.cursor, forward.request.endTs);
const nextForward = planNextForwardBarRequest(moved);
assert.equal(nextForward.request.start, '2026-06-01 21:56');
assert.equal(nextForward.request.end, '2026-06-01 21:57');
...
const updatedActive = updateActiveReplaySession({ cursor: forward.request.endTs });
assert.equal(updatedActive.cursor, forward.request.endTs);
clearActiveReplaySession();
assert.equal(hasActiveReplaySession(), false);
