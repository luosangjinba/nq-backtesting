...
assert.deepEqual(moved, { x: 0, y: 0, width: 50, height: 40 });

const reset = resetComparisonVisibleWindow();
assert.deepEqual(reset, { x: 0, y: 0, width: 34, height: 100 });

const descriptor = getComparisonViewDescriptor();
descriptor.visibleWindow.x = 1;
assert.equal(getComparisonWindowState().descriptor.visibleWindow.x, 0);
