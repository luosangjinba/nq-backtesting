#!/usr/bin/env python3
"""Preview, commit, or verify one reviewed historical roll repair manifest."""
...
except ImportError:  # pragma: no cover - commit and verify do not need Databento
    db = None
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Guarded manifest-driven historical roll repair.")
    parser.add_argument("--plan", required=True, help="Reviewed repair manifest path.")
    ...
    parser.add_argument(
        "--repair-root",
        default=str(repair_service.default_repair_root()),
        help="Staging/backup/audit root.",
    )
    mode = parser.add_mutually_exclusive_group()
    mode.add_argument("--commit", action="store_true", help="Commit a retained preview.")
    mode.add_argument("--verify", action="store_true", help="Verify committed state.")
...
def download_preview_inputs(
    plan: repair_service.RepairPlan,
) -> tuple[dict[str, pd.DataFrame], dict[str, list[dict[str, str | None]]]]:
    if db is None:
...
def main() -> int:
    args = parse_args()
    plan = repair_service.load_plan(args.plan)
    ...
    if args.verify:
        result = repair_service.verify_repair(plan, database, calendar)
    elif args.commit:
        ...
        replacements, conditions = download_preview_inputs(plan)
...
if __name__ == "__main__":
    try:
        ...
        print("manifest_roll_repair_status: failed")
        print(f"error: {exc}")
        raise SystemExit(1)
