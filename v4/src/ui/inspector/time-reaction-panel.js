...
import {
  CHART_NOTES_SECTION_KEY,
  renderChartNotesSection,
} from './time-reaction-chart-notes-section.js';

export { CHART_NOTES_SECTION_KEY } from './time-reaction-chart-notes-section.js';
...
function renderChartOptions(selectedChart) {
  ...
  return ['primary', 'comparison-window']
    .map((value) => `
      <option value="${escapeHtml(value)}" ${value === selected ? 'selected' : ''}>${escapeHtml(value === 'primary' ? 'Pane 2' : 'Pane 1')}</option>
    `)
    .join('');
}
