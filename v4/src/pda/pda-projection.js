...
export function getPdaProjectionTimestamps(annotation = {}) {
  ...
  pushTimestamp(timestamps, annotation.startTimeTimestamp);
  pushTimestamp(timestamps, annotation.endTimeTimestamp);
  pushTimestamp(timestamps, annotation.start?.timestamp);
  pushTimestamp(timestamps, annotation.start?.time);
  pushTimestamp(timestamps, annotation.end?.timestamp);
  pushTimestamp(timestamps, annotation.end?.time);
  ...
  (Array.isArray(annotation.points) ? annotation.points : []).forEach((point) => {
    ...
    pushTimestamp(timestamps, point?.anchorTime);
