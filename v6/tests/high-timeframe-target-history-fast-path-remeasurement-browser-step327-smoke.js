...
try {
  const value = JSON.parse(await evaluate(page.client, `
    (async () => JSON.stringify(await (async () => {
      ...
      const unsubscribeDisplay = events.subscribeEvent(
        contracts.DISPLAY_TIMEFRAME_EVENTS.APPLIED,
        (record = {}) => {
          if (record.pane?.id !== 'main') return;
          mark('display-timeframe-applied-event', {
            displayTimeframe: record.pane.displayTimeframe,
          });
        },
      );
      const unsubscribeHistory = events.subscribeEvent(
        ...
        (extension = {}) => {
          ...
          mark('left-extension-loaded', {
            durationMs: extension.diagnostics.durationMs,
      ...
      async function collectSample({ expectedTargetFetchTf, targetTimeframe }) {
        ...
        mark('display-apply-returned', { targetTimeframe });
        ...
        while (performance.now() < deadline) {
          ...
          if (
            firstMilestone(milestones, 'target-fetch-started') &&
            firstMilestone(milestones, 'target-fetch-ended') &&
        ...
        const targetFetch = targetFetches.at(-1) || null;
        ...
        const applyStartedAt = named['target-history-apply-start']?.time ?? performance.now();
        const displayApplyReturnedAt = named['display-apply-returned']?.time ?? applyStartedAt;
        const targetFetchStartedAt = named['target-fetch-started']?.time ?? targetFetch?.startedAt ?? displayApplyReturnedAt;
        const targetFetchEndedAt = named['target-fetch-ended']?.time ?? targetFetch?.endedAt ?? targetFetchStartedAt;
        const chartDataAppliedAt = named['chart-data-applied']?.time ?? targetFetchEndedAt;
        const leftExtensionLoadedAt = named['left-extension-loaded']?.time ?? chartDataAppliedAt;
        const readoutVisibleAt = named['diagnostics-readout-visible']?.time ?? leftExtensionLoadedAt;
        return {
          applyLagMs: Math.max(0, readoutVisibleAt - leftExtensionLoadedAt),
          browserVisible: after.readout.path === 'target',
          displayApplyMs: Math.max(0, displayApplyReturnedAt - applyStartedAt),
          displayTimeframe: targetTimeframe,
          durationMs: Number(diagnostics.durationMs),
          eventEmissionMs: Math.max(0, leftExtensionLoadedAt - chartDataAppliedAt),
          fallbackReason: diagnostics.fallbackReason || null,
          fetchMs: targetFetch?.durationMs ?? Math.max(0, targetFetchEndedAt - targetFetchStartedAt),
          measurementBoundary: 'trigger-coordination-milestones',
          milestones,
          path: diagnostics.path || null,
          postApplyTargetFetchStartMs: Math.max(0, targetFetchStartedAt - displayApplyReturnedAt),
          preLeftExtensionMs: Math.max(0, leftExtensionLoadedAt - applyStartedAt),
          ...
          targetRuntimeMs: Math.max(0, leftExtensionLoadedAt - targetFetchStartedAt),
          visualLatencyMs: Math.max(0, readoutVisibleAt - applyStartedAt),
        };
      }
  ...
  value.records.forEach((record, index) => {
    ...
    assert.equal(record.measurementBoundary, 'trigger-coordination-milestones');
    ...
    [
      ...
      assert.equal(Number.isFinite(named[name]?.time), true, name);
    });
    assert.equal(named['target-history-apply-start'].time <= named['display-apply-returned'].time, true);
    assert.equal(named['display-apply-returned'].time <= named['target-fetch-started'].time, true);
    ...
    assert.equal(Number.isFinite(record.displayApplyMs), true);
    assert.equal(Number.isFinite(record.postApplyTargetFetchStartMs), true);
    assert.equal(Number.isFinite(record.targetRuntimeMs), true);
    assert.equal(Number.isFinite(record.eventEmissionMs), true);
    assert.equal(Number.isFinite(record.preLeftExtensionMs), true);
  });

  const remeasurement = remeasureHighTimeframeTargetHistoryFastPathResponsiveness({
    records: value.records,
  });
  assert.equal(remeasurement.status, 'materialization-ready');
  assert.equal(remeasurement.ownerBoundary, 'target-history-fast-path-responsive');
  assert.equal(remeasurement.nextSlice, 'replay-coordination-materialization-transition');
  assert.equal(remeasurement.audit.summary.browserSampleCount, 3);
  assert.equal(remeasurement.audit.summary.targetCount, 3);
  assert.equal(remeasurement.audit.summary.fallbackRate, 0);
  assert.equal(remeasurement.audit.summary.applyLagP95Ms < remeasurement.audit.thresholds.maxApplyLagMs, true);
  assert.equal(remeasurement.audit.summary.visualLatencyP95Ms < remeasurement.audit.thresholds.maxVisualLatencyMs, true);
  assert.equal(value.records.every((record) => record.preLeftExtensionMs < remeasurement.audit.thresholds.maxVisualLatencyMs), true);
  assert.equal(value.restored.sourceBarCount, value.initial.sourceBarCount);
} finally {
  await page.cleanup();
}

console.log('v6 high timeframe target history fast path remeasurement browser step327 smoke passed');
