...
const forwardCursorResolver = await readFile('v6/src/replay/replay-forward-source-cursor-resolver.js', 'utf8');
const cursorPaneMaterializer = await readFile('v6/src/replay/replay-cursor-pane-materializer.js', 'utf8');
...
assert.match(manualNextRuntime, /advanceReplayToNextSourceBar/);
assert.match(manualNextRuntime, /appendReplayCursorAcrossPanes/);
assert.match(forwardCursorResolver, /SET_CURSOR_TIME/);
assert.match(forwardCursorResolver, /LOAD_WINDOW/);
assert.match(cursorPaneMaterializer, /CHART_DATA_PROJECTION_COMMANDS\.PROJECT/);
assert.match(cursorPaneMaterializer, /CHART_DATA_COMMANDS\.APPEND_BARS/);
