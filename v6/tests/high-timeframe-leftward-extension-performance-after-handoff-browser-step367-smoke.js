...
try {
  const value = JSON.parse(await evaluate(page.client, `
    (async () => JSON.stringify(await (async () => {
      ...
      function largestPhase(phaseBreakdown) {
        const phases = [
          'sourceRequestMs',
          'targetRequestMs',
        ...
        return phases.reduce((largest, phase) => {
          const value = Number(phaseBreakdown[phase] || 0);
          return value > largest.durationMs ? { durationMs: value, phase } : largest;
        }, { durationMs: -1, phase: null });
      }
      ...
      async function collectSample({ expectedTargetFetchTf, label, targetTimeframe }) {
        ...
        const phaseBreakdown = {
          browserPaintLagMs: Math.max(0, secondAnimationFrameAt - readoutVisibleAt),
          chartDataReplacementMs: Math.max(0, chartDataAppliedAt - requestEndedAt),
          runtimeDurationMs: Number(diagnostics.durationMs),
          ...
          visibleApplyLagMs: Math.max(0, readoutVisibleAt - leftExtensionLoadedAt),
          visualLatencyMs: Math.max(0, readoutVisibleAt - startedAt),
        };
        return {
          bottleneckHint: largestPhase(phaseBreakdown),
          browserVisible: after.readout.path === 'target',
          displayTimeframe: targetTimeframe,
          expectedTargetFetchTf,
          fallbackReason: diagnostics.fallbackReason || null,
          label,
          measurementBoundary: 'htf-leftward-extension-performance-after-handoff',
          milestones,
  ...
  value.records.forEach((record) => {
    ...
    assert.equal(record.measurementBoundary, 'htf-leftward-extension-performance-after-handoff');
    ...
    assert.equal(Number.isFinite(named['display-apply-returned']?.time), true);
    ...
    assert.equal(named['diagnostics-readout-visible'].time <= named['second-animation-frame-after-readout'].time, true);
    assert.ok(record.bottleneckHint.phase);
    assert.equal(Number.isFinite(record.bottleneckHint.durationMs), true);
  });
...
console.log('v6 high timeframe leftward extension performance after handoff browser step367 smoke passed');
