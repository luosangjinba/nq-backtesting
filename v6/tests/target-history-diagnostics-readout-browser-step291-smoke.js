...
try {
  ...
  assert.equal(value.after.readout.sourceRequests, '0');
  assert.equal(value.after.readout.prependedBars, String(diagnostics.prependedBarCount));
  assert.match(value.after.readout.text, /^History target \d+ms T1\/S0 \+\d+$/);
  assert.match(value.after.readout.title, /Path: target-history/);
  assert.match(value.after.readout.title, /Fallback reason: none/);
...
console.log('v6 target history diagnostics readout browser step291 smoke passed');
