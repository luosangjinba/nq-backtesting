...
const sourceBars = [
  ...
  bar(ts('2026-06-07T17:59:00Z'), 2),
  bar(ts('2026-06-07T18:00:00Z'), 3),
  bar(ts('2026-06-14T17:59:00Z'), 4),
];
...
assert.equal(projection.targetTimeframe, '1W');
assert.deepEqual(projection.bars, [
  ...
  },
  {
    ...
    low: 2,
    open: 3.25,
    timestamp: ts('2026-06-07T18:00:00Z'),
  },
]);
assert.deepEqual(projection.buckets.map((bucket) => ({
  ...
  },
  {
    complete: true,
    cursorCapped: false,
    end: ts('2026-06-14T17:59:59Z'),
    key: '2026-06-08',
    sourceCount: 2,
    start: ts('2026-06-07T18:00:00Z'),
    unit: 'week',
  },
]);
...
const cursorProjection = projectSourceBarsToChartData({
  bars: sourceBars,
  cursorTimestamp: ts('2026-06-07T18:00:00Z'),
  instrument: 'NQ',
  sourceTimeframe: 1,
  targetTimeframe: '1W',
});
...
console.log('v6 weekly projection domain step269 smoke passed');
