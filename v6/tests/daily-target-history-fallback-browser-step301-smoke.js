  ...
  const value = JSON.parse(await evaluate(page.client, `
    (async () => JSON.stringify(await (async () => {
      ...
      function makeBars({ end, start, stepSeconds }) {
        ...
        const bars = [];
        for (let timestamp = startTs; timestamp <= endTs && bars.length < 16000; timestamp += stepSeconds) {
...
console.log('v6 daily target history fallback browser step301 smoke passed');
