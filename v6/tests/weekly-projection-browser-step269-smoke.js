...
try {
  const value = JSON.parse(await evaluate(page.client, `
    (async () => JSON.stringify(await (async () => {
      ...
      document.querySelector('[data-v6-display-timeframe-option="1W"]').click();
      ...
      while ((projectionState.lastProjection?.targetTimeframe !== '1W' || chartRecord.bars.length !== 2) && performance.now() < deadline) {
      ...
      return {
        ...
        hasWeeklyOption: Boolean(document.querySelector('[data-v6-display-timeframe-option="1W"]')),
  ...
  assert.equal(value.hasWeeklyOption, true);
  assert.equal(value.rootDataset, '1W');
  assert.equal(value.label, '1W');
  assert.equal(value.menuOpen, 'false');
  assert.equal(value.pane.displayTimeframe, '1W');
  assert.deepEqual(value.bars.map((bar) => ({
    ...
    },
    {
      ...
      low: 198,
      open: 200,
      timestamp: Date.parse('2026-06-07T18:00:00Z') / 1000,
    },
  ]);
  assert.equal(value.projection.sourceTimeframe, 1);
  assert.equal(value.projection.targetTimeframe, '1W');
  assert.equal(value.projection.buckets[0].key, '2026-06-01');
  assert.equal(value.projection.buckets[1].key, '2026-06-08');
} finally {
  await page.cleanup();
}
