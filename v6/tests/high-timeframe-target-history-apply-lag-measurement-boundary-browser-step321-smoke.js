...
try {
  ...
  const residualVisualLatencySlices = new Set([
    'target-history-chart-data-replacement-optimization',
    'target-history-viewport-reapply-optimization',
  ]);
  const residualVisualLatencyPhases = new Set([
    'chart-data-replacement',
    'viewport-reapply',
  ]);
  assert.equal(selection.status, 'optimize-phase');
  assert.equal(residualVisualLatencySlices.has(selection.selectedSlice), true);
  assert.equal(residualVisualLatencyPhases.has(selection.selectedPhase), true);
...
console.log('v6 high timeframe target history apply lag measurement boundary step321 smoke passed');
