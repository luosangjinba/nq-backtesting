...
for (const script of TESTS) {
  console.log(`[replay-gap-browser-pack] start ${script}`);
  const result = await runTest(script);
  results.push(result);
  const status = result.code === 0 ? 'pass' : 'fail';
  console.log(`[replay-gap-browser-pack] ${status} ${script} ${result.durationMs}ms`);
  if (result.code !== 0) {
    break;
  }
}
...
if (failed) {
  console.error('[replay-gap-browser-pack] failed');
  console.error(JSON.stringify(failed, null, 2));
  process.exit(failed.code || 1);
}

const totalMs = results.reduce((sum, result) => sum + result.durationMs, 0);
console.log(`[replay-gap-browser-pack] passed ${results.length}/${TESTS.length} in ${totalMs}ms`);
