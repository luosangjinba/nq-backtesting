...
try {
  ...
  const selection = selectHighTimeframeTargetHistoryPhaseBudget({
    phaseBudgets: {
      'browser-visible-apply-lag': 10000,
      'chart-data-replacement': 10000,
      fetch: 10000,
      'viewport-reapply': 10000,
    },
  ...
  assert.equal(selection.status, 'materialization-ready');
  assert.equal(selection.selectedPhase, null);
  assert.equal(selection.selectedSlice, 'replay-coordination-materialization-transition');
...
console.log('v6 high timeframe target history browser phase budget selection step316 smoke passed');
