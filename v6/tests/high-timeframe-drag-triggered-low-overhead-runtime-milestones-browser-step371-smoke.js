...
try {
  const setup = await pageEval(`
        ...
        for (const canvas of canvases) {
          ...
          for (let y = 0; y < height; y += stepY) {
            for (let x = 0; x < width; x += stepX) {
              ...
              if (alpha > 0 && (greenCandle || redCandle)) candlePixelCount += 1;
      ...
      window.fetch = async (input, init) => {
        ...
        if (url.pathname === '/v4/bars') {
          mark('source-fetch-started', { tf: url.searchParams.get('tf') });
          ...
          mark('source-fetch-ended', { bars: bars.length, tf: url.searchParams.get('tf') });
        ...
        if (url.pathname === '/v4/target_bars') {
          ...
          mark('target-fetch-ended', { bars: bars.length, tf });
          return new Response(JSON.stringify({
            bars,
            requestedRange: {
              end: url.searchParams.get('end'),
      ...
      window.__v6Step371 = {
        ...
        async waitForAttempt({ timeoutMs = 1600 } = {}) {
          ...
          while (performance.now() < deadline) {
            ...
            if (
              firstMilestone('target-fetch-ended') &&
          ...
          const postMarkerCanvasStartedAt = performance.now();
          const postMarkerCanvasSignature = canvasSignature();
          const postMarkerCanvasObservationMs = performance.now() - postMarkerCanvasStartedAt;
          after = await snapshot();
          ...
          const targetFetchEnded = firstMilestone('target-fetch-ended');
          ...
          return {
            ...
            loaded,
            measurementBoundary: 'htf-drag-triggered-low-overhead-runtime-milestone-attribution',
            milestones: activeMilestones || [],
            path: diagnostics.path || null,
            phaseBreakdown: {
              chartDataToViewportMs: delta(viewportProjected, chartDataApplied),
              fetchEndToChartDataMs: delta(chartDataApplied, targetFetchEnded),
              inputToTargetFetchStartMs: delta(targetFetchStarted, input),
              leftExtensionToReadoutMs: delta(readoutVisible, leftExtensionLoaded),
              sourceRequestMs: requestMs(sourceFetches),
              targetFetchStartToEndMs: delta(targetFetchEnded, targetFetchStarted),
              targetRequestMs: requestMs(targetFetches),
              viewportToLeftExtensionMs: delta(leftExtensionLoaded, viewportProjected),
            },
            ...
            viewportProjectionObserved: Boolean(viewportProjected),
          };
        },
        async restore() {
  ...
  for (const sample of samples) {
    const prepared = await pageEval(`
      return await window.__v6Step371.prepareSample(${JSON.stringify(sample)});
    `);
    ...
    for (const [attemptIndex, deltaX] of [-960, 960, -1280, 1280, -1600, 1600, -2200, 2200].entries()) {
      await pageEval(`
        return window.__v6Step371.beginInputAttempt(${JSON.stringify({ attemptIndex, deltaX })});
      `);
      ...
      const attempt = await pageEval(`
        return await window.__v6Step371.waitForAttempt({ timeoutMs: 1800 });
      `);
      if (attempt.loaded) {
        record = attempt;
        break;
  ...
  const restored = await pageEval(`
    return await window.__v6Step371.restore();
  `);
  ...
  records.forEach((record) => {
    ...
    assert.equal(record.measurementBoundary, 'htf-drag-triggered-low-overhead-runtime-milestone-attribution');
    assert.equal(['left-extension-listener', 'poll'].includes(record.readoutObservation), true);
    ...
    for (const key of [
      ...
      assert.equal(Number.isFinite(record.phaseBreakdown[key]), true, key);
      assert.equal(record.phaseBreakdown[key] >= 0, true, key);
    }
    for (const key of ['chartDataToViewportMs', 'viewportToLeftExtensionMs']) {
      assert.equal(
        record.phaseBreakdown[key] === null || Number.isFinite(record.phaseBreakdown[key]),
        true,
        key,
      );
    }
    assert.equal(Number.isFinite(record.postMarkerCanvasObservationMs), true);
    assert.equal(record.postMarkerCanvasSignature.canvasCount > 0, true);
    assert.equal(record.postMarkerCanvasSignature.sampledPixelCount > 0, true);
    ...
    assert.equal(
      named['viewport-projected'] === undefined || Number.isFinite(named['viewport-projected']?.time),
      true,
    );
    ...
    if (named['viewport-projected']) {
      assert.equal(named['viewport-projected'].time <= named['left-extension-loaded'].time, true);
    }
...
console.log('v6 high timeframe drag-triggered low-overhead runtime milestones browser step371 smoke passed');
