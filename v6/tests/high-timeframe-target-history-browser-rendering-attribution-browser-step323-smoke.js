...
try {
  const value = JSON.parse(await evaluate(page.client, `
    (async () => JSON.stringify(await (async () => {
      ...
      async function collectSample({ expectedTargetFetchTf, targetTimeframe }) {
        ...
        const firstAnimationFrameAt = named['first-animation-frame-after-readout']?.time ?? readoutVisibleAt;
        const secondAnimationFrameAt = named['second-animation-frame-after-readout']?.time ?? firstAnimationFrameAt;
        ...
        return {
          ...
          leftExtensionLoadedAt,
          measurementBoundary: 'browser-rendering-visibility-milestones',
          milestones,
          path: diagnostics.path || null,
          postLeftExtensionReadoutMs: Math.max(0, readoutVisibleAt - leftExtensionLoadedAt),
          postLeftExtensionSecondFrameMs: Math.max(0, secondAnimationFrameAt - leftExtensionLoadedAt),
          postReadoutSecondFrameMs: Math.max(0, secondAnimationFrameAt - readoutVisibleAt),
          preLeftExtensionMs: Math.max(0, leftExtensionLoadedAt - startedAt),
  ...
  value.records.forEach((record, index) => {
    ...
    assert.equal(record.measurementBoundary, 'browser-rendering-visibility-milestones');
    ...
    assert.equal(Number.isFinite(named['target-history-apply-start']?.time), true);
    ...
    assert.equal(Number.isFinite(named['first-animation-frame-after-readout']?.time), true);
    ...
    assert.equal(named['diagnostics-readout-visible'].time <= named['first-animation-frame-after-readout'].time, true);
    assert.equal(named['first-animation-frame-after-readout'].time <= named['second-animation-frame-after-readout'].time, true);
    assert.equal(record.applyLagMs, record.postLeftExtensionReadoutMs);
    assert.equal(Number.isFinite(record.preLeftExtensionMs), true);
    assert.equal(Number.isFinite(record.postLeftExtensionSecondFrameMs), true);
    assert.equal(Number.isFinite(record.postReadoutSecondFrameMs), true);
    assert.equal(Number.isFinite(record.visualLatencyMs), true);
  });

  const attribution = attributeHighTimeframeTargetHistoryBrowserRenderingVisibility({
    records: value.records,
  });
  assert.equal(attribution.status, 'trigger-coordination-attribution-needed');
  assert.equal(attribution.ownerBoundary, 'target-history-trigger-or-coordination-boundary');
  assert.equal(attribution.nextSlice, 'target-history-trigger-coordination-latency-attribution');
  assert.equal(attribution.reason, 'pre-left-extension-window-dominates-visual-latency');
...
console.log('v6 high timeframe target history browser rendering attribution browser step323 smoke passed');
