...
try {
  const value = JSON.parse(await evaluate(page.client, `
    (async () => JSON.stringify(await (async () => {
      ...
      async function collectSample({ expectedTargetFetchTf, targetTimeframe }) {
        ...
        const fetchMs = targetFetch?.durationMs ?? null;
  ...
  assert.equal(probe.budgetReport.summary.browserSampleCount, 3);
  assert.equal(probe.budgetReport.summary.targetCount, 3);
  assert.equal(probe.budgetReport.summary.fallbackRate, 0);
  assert.equal(probe.reason, 'target-history-runtime-optimization-not-needed');
  assert.equal(probe.nextSlice, 'replay-coordination-materialization-transition');
  assert.equal(value.restored.sourceBarCount, value.initial.sourceBarCount);
} finally {
  await page.cleanup();
}

console.log('v6 high timeframe target history browser phase timing step314 smoke passed');
