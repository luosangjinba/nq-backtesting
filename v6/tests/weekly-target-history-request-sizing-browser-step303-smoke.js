...
try {
    ...
    (async () => JSON.stringify(await (async () => {
      ...
      window.fetch = async (input, init) => {
        ...
        if (url.pathname === '/v4/target_bars') {
          ...
          const stepSeconds = tf === '1W' ? 7 * 24 * 60 * 60 : 24 * 60 * 60;
          const bars = makeBars({
            end: url.searchParams.get('end'),
            start: url.searchParams.get('start'),
            stepSeconds,
  ...
  assert.equal(value.after.displayTimeframe, '1W');
  assert.equal(Boolean(targetFetch), true);
  assert.equal(targetFetch.tf, '1W');
  ...
  assert.equal(value.after.requestSizing.targetDisplayBars, 4);
...
console.log('v6 weekly target history request sizing browser step303 smoke passed');
