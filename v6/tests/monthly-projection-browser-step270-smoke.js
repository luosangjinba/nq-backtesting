...
try {
  const value = JSON.parse(await evaluate(page.client, `
    (async () => JSON.stringify(await (async () => {
      ...
      document.querySelector('[data-v6-display-timeframe-option="1M"]').click();
      ...
      while ((projectionState.lastProjection?.targetTimeframe !== '1M' || chartRecord.bars.length !== 2) && performance.now() < deadline) {
      ...
      return {
        ...
        hasMonthlyOption: Boolean(document.querySelector('[data-v6-display-timeframe-option="1M"]')),
  ...
  assert.equal(value.hasMonthlyOption, true);
  assert.equal(value.rootDataset, '1M');
  assert.equal(value.label, '1M');
  assert.equal(value.menuOpen, 'false');
  assert.equal(value.pane.displayTimeframe, '1M');
  assert.deepEqual(value.bars.map((bar) => ({
    ...
    },
    {
      ...
      low: 198,
      open: 200,
      timestamp: Date.parse('2026-06-30T18:00:00Z') / 1000,
    },
  ]);
  assert.equal(value.projection.sourceTimeframe, 1);
  assert.equal(value.projection.targetTimeframe, '1M');
  assert.equal(value.projection.buckets[0].key, '2026-06');
  assert.equal(value.projection.buckets[1].key, '2026-07');
} finally {
  await page.cleanup();
}
