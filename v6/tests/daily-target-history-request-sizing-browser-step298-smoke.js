...
try {
    ...
    (async () => JSON.stringify(await (async () => {
      ...
      window.fetch = async (input, init) => {
        ...
        if (url.pathname === '/v4/target_bars') {
          ...
          const stepSeconds = tf === '1D' ? 24 * 60 * 60 : 8 * 60 * 60;
          const bars = makeBars({
            end: url.searchParams.get('end'),
            start: url.searchParams.get('start'),
            stepSeconds,
  ...
  assert.equal(targetFetch.tf, '1D');
  ...
  assert.equal(diagnostics.path, 'target-history');
  ...
  assert.equal(value.after.requestSizing.targetDisplayBars, 12);
  assert.equal(value.after.requestSizing.policy.prefetchSourceBars, 17280);
  assert.equal(value.after.history.extension.targetHistory.barCount, targetFetch.bars);
  assert.equal(value.after.history.extension.prependedBarCount, targetFetch.bars);
...
console.log('v6 daily target history request sizing browser step298 smoke passed');
