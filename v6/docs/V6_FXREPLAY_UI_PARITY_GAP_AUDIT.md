...
| --- | --- | --- | --- | --- |
| Session dashboard | Standalone session selector surface with Backtesting session creation, Sessions list, and Analytics placeholder; the app boots here before chart entry | Simplified FXReplay-like session selector distinct from chart workstation | shell-only UI complete for first slice; durable routing, persistence, session-segment analytics, replay orders, and live orders remain deferred | Keep session surface separate from chart; do not reintroduce overlay semantics, chart-forward arrow, generic Dashboard overview, Tutorials, Prop firm entry, promotion panels, performance charts, or chart/replay ownership |
