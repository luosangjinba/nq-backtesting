...
| --- | --- | --- | --- | --- |
| Session dashboard | Separate shell selector opened from the chart back arrow with only Backtesting session creation, Sessions list, and Analytics placeholder | Simplified FXReplay-like session selector distinct from chart workstation | shell-only UI complete for first slice; durable routing, persistence, session-segment analytics, replay orders, and live orders remain deferred | Keep dashboard separate from chart; do not reintroduce chart-forward arrow, generic Dashboard overview, Tutorials, Prop firm entry, promotion panels, performance charts, or chart/replay ownership |
