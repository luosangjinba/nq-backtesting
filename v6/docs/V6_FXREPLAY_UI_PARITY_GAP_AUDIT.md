...
| --- | --- | --- | --- | --- |
| Session dashboard | Separate shell dashboard/list surface opened from the chart back arrow; session creation/list display uses session commands only | FXReplay-like session dashboard/list distinct from chart workstation | shell-only UI complete for first slice; durable routing, persistence, statistics, and account analytics remain deferred | Keep dashboard separate from chart; do not reintroduce chart-forward arrow or chart/replay ownership |
