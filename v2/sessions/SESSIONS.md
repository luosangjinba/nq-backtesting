...
**背景：**
kline_viewer 的 Manual PDA 匹配功能需要重构：旧的匹配合并功能块过于复杂，需要简化为纯匹配预览；同时切换周期后 PDA 定位偏移是核心 UX 问题。

**改动内容：**

1. **匹配功能重构**
   - 移除旧的匹配合并功能块（安全版应用合并、Merge YAML 等），简化为纯匹配预览
   - 修复 `/v2/pda_match` API 参数顺序错误
   - 修复 FVG/OB 等区间类型 PDA 的匹配逻辑（区间类型用 price_high/price_low 匹配）
   - 匹配结果列出所有相关周期和类型（不再只显示单一匹配）

2. **点击匹配结果采用 anchor_time**
   - 点击匹配表格中的行，自动将该自动 PDA 的 anchor_time 填入表单
   - 刷新预览位置，解决切换 TF 后预览偏移问题

3. **跨周期 PDA 定位（核心难题）**
   - 问题：切换周期后，PDA overlay 定位到错误位置（因为不同周期的 K 线索引不同）
   - 经历多轮尝试：
     - 保存/恢复 currentPda → 不够
     - 用价格匹配新周期 K 线 → 基本可行
     - 重构为价格匹配 K 线实现跨周期定位
     - 修复周期字符串解析计算时间范围
     - 支持手动预览 PDA 的跨周期匹配
   - 最终方案：在1H范围内找低周期极值，用价格匹配定位

**修改文件：**
- `v2/docs/kline_viewer.html` — 匹配重构、跨周期定位
- `price_lookup_api.py` — pda_match 参数修复、区间类型匹配逻辑

**当前状态：**
- 跨周期定位基本可用，使用时间范围约束 + 价格匹配
- 匹配预览简化为只读展示，不再有合并/写库操作

---

## 2026-05-10

### 会话 15：OB 矩形框修复 + FVG 显示优化

**背景：**
OB overlay 绘制不正确，FVG 显示方式需要迭代优化。
...
2. **FVG 显示方式迭代**
   - 双虚线边界 → 左右开口矩形 + 中央标签 → 实线边界 + 中线
   - 最终方案：实线上下边界 + 中线，中央显示周期标签

**修改文件：**
- `v2/docs/kline_viewer.html` — OB 绘制修复、FVG 外观优化

---

### 会话 14：Lightweight Charts 迁移尝试与回退

**背景：**
尝试将 KLineChart 迁回 TradingView Lightweight Charts（因为 LC 的渲染质量更好），但最终因 overlay 能力不足而回退。

**结论：**
- Lightweight Charts 仍然无法满足 FVG/OB 矩形填充需求
- 保持 KLineChart v9.8.12

---

### 会话 13：多 PDA 预览 + FVG 扩展 + 午夜线标签探索

**背景：**
继续完善 kline_viewer 的 PDA 标注体验和图表信息密度。

**改动内容：**

1. **Manual PDA 预览优化**
   - 预览标注时保持图表位置不变（不再自动居中）
   - 修复 PDA Badge 撑高工具栏导致页面抖动
   - 右键菜单文案改为"手动标记 PDA"
   - 右键手动标记后立即显示预览

2. **多 PDA 标注同时显示（方案 A）**
   - 支持多个 Manual PDA 预览同时在图表上显示
   - 不再每次预览新 PDA 时清除旧的

3. **FVG 扩展功能**
   - 右键菜单控制 FVG overlay 向右扩展
   - 修复扩展到最右端后隐性累加问题

4. **午夜线日期标签（尝试后回退）**
   - 尝试在午夜线旁添加日期/时间/周标签
   - 发现 KLineChart 不支持 text overlay 类型
   - 回退：十字光标和 X 轴标签已提供足够时间信息

**修改文件：**
- `v2/docs/kline_viewer.html` — 多 PDA 预览、FVG 扩展、午夜线标签尝试与回退
