...
### 背景
完成 Git 分支整理后，开始新功能开发：图表显示控制（K 线回放 + PDA 显示控制 + 标签显示控制）。

### 执行的操作

**1. 创建新分支**：
```bash
git checkout -b feature/chart-display-control
```

**2. 制定功能计划**：
- 创建 `v3/docs/CHART_DISPLAY_CONTROL_PLAN.md`
- 三个核心功能方向：
  1. K 线回放（Replay）- 1.5 天
  2. PDA 显示控制 - 1.5 天
  3. 标签显示控制 - 1 天

**3. 参考设计**：
- 最佳方案：FXReplay 浮动工具条（半透明，浮动在图表中央底部）
- 次选方案：TradingView 固定底部工具条

**4. 提交计划文档**：
```
26b8526 - docs: 添加图表显示控制功能计划
```
...
#### 功能 2：PDA 显示控制
- 按类型过滤（BSL/SSL/FVG/NWOG/NDOG/Daily H/L/ICT Midnight H/L/EQH/EQL）
- 按周期过滤（1M/5M/15M/1H/4H/D/W）
- 多周期叠加显示
- 周期切换时 PDA 保持
- 按时段过滤（时间范围/日期/交易时段）
...
### 下一步
- 开始实施阶段 1：K 线回放功能
- 参考文档：`v3/docs/CHART_DISPLAY_CONTROL_PLAN.md`
