...
const DEBUG_PORT = Number(process.env.CHROME_DEBUG_PORT || 9420);
const PROFILE_DIR = process.env.CHROME_PROFILE_DIR || `/tmp/v5-triple-pane-initial-coverage-${process.pid}`;
  ...
  try {
    ...
    const value = JSON.parse(await evaluate(client, `
      (async () => {
        ...
        async function layoutFrames(commands) {
          const layout = await commands.dispatchCommand('layout.getState');
          return Object.fromEntries(layout.panes.map((pane) => [
            pane.id,
        ...
        try {
          ...
          const created = await commands.dispatchCommand('session.create', {
            id: 'browser-triple-pane-initial-coverage',
            instrument: 'NQ',
            timeframe: 1,
          ...
          await waitFor('secondary 5m ready', async () => (
          ...
          timeframeSelect.dispatchEvent(new Event('change', { bubbles: true }));
          await waitFor('primary 1H coverage with independent panes', async () => {
            const frames = await layoutFrames(commands);
            return frames.primary === 60
              && frames.secondary === 5
              && frames.tertiary === 1
              && paneState('primary').displayTimeframe === '60'
              && paneState('primary').renderedBarCount >= minimumCoverage
              && paneState('secondary').displayTimeframe === '5'
          ...
          return JSON.stringify({
            ...
            targetVisibleBars,
            minimumCoverage,
            frames: await layoutFrames(commands),
            primary: paneState('primary'),
            secondary: paneState('secondary'),
            tertiary: paneState('tertiary'),
            requests,
    ...
    assert.deepEqual(value.frames, { primary: 60, secondary: 5, tertiary: 1 });
    assert.ok(
      value.primary.renderedBarCount >= value.minimumCoverage,
      `expected primary coverage >= ${value.minimumCoverage}: ${JSON.stringify(value)}`
    );
    assert.equal(value.secondary.displayTimeframe, '5');
    assert.equal(value.tertiary.displayTimeframe, '1');
    assert.ok(value.requests.filter((request) => request.timeframe === 60).length >= 2);
  } finally {
    if (client) {
      try {
        await client.close();
...
console.log('v5 triple pane initial coverage browser smoke passed');
