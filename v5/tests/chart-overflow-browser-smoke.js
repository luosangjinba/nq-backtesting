...
const DEBUG_PORT = Number(process.env.CHROME_DEBUG_PORT || 9368);
const PROFILE_DIR = process.env.CHROME_PROFILE_DIR || `/tmp/v5-chart-overflow-browser-smoke-${process.pid}`;
    ...
    const value = JSON.parse(await evaluate(client, `
      (async () => {
        ...
        const created = await commands.dispatchCommand('session.create', {
          id: 'browser-chart-overflow',
          instrument: 'NQ',
          timeframe: 1,
          sessionStart: '2025-06-02 10:00',
          sessionEnd: '2025-06-02 10:30',
        });
        ...
        return JSON.stringify({
          ...
          barCount: Number(document.querySelector('[data-chart-bar-count]')?.dataset.chartBarCount || 0),
        });
      })()
    `));

    assert.ok(value.barCount > 1, 'chart should render replay bars');
    assert.ok(value.scrollWidth <= value.clientWidth, `document overflowed: ${JSON.stringify(value)}`);
    assert.ok(value.bodyScrollWidth <= value.clientWidth, `body overflowed: ${JSON.stringify(value)}`);
    assert.ok(value.chartRight <= value.viewportWidth, `chart overflowed: ${JSON.stringify(value)}`);
  } finally {
    client?.close();
    chrome.kill('SIGTERM');
...
main().then(
  () => console.log('v5 chart overflow browser smoke passed'),
  (error) => {
    console.error(error);
    process.exit(1);
  }
